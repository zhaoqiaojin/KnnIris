...
get_neighbors_classes([_-Class|T], TrainData, [Class|Classes]) :-
    member(data(X1,Y1,Z1,W1,Class), TrainData),
    data(X2,Y2,Z2,W2,_), % dummy test instance
    distance(X1,Y1,Z1,W1,X2,Y2,Z2,W2,0), % use the distance predicate to unify X1,Y1,Z1,W1 with the neighbor instance
